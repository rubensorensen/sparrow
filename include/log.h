...
const char *_get_log_time_string(void);

#define NONCOLERED_LOG_BASE(sink, log_type_string, ...) {   \
        fprintf(sink, "(%s) %-7s %s:%-5d  ",                \
                _get_log_time_string(),                     \
                log_type_string,                            \
                FILENAME,                                   \
                FILELINE);                                  \
...
#define COLORED_LOG_BASE(sink, log_type_string,                 \
                         ...
                         file_line_color,                       \
                         text_color, ...) {                     \
        fprintf(sink, "(%s%s%s) %s%-7s%s %s%s%s:%s%-5d%s  ",    \
                ANSI_WHITE, _get_log_time_string(), ANSI_RESET, \
...
b32 _check_terminal_supports_ansi_escape_codes(void);

#define LOG_TRACE(...) {                                    \
        if (_check_terminal_supports_ansi_escape_codes()){  \
